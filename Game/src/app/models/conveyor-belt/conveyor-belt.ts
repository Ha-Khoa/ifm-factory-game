...
//    }
   ...
   getItemPosition(progress: number): Coordinates{
       let x = this.x;
       let y = this.y;
   ...
   getProductPosition(progress: number): Coordinates{
         return this.getItemPosition(progress);
   }
   private updateItemPositions(): void{
       this._items.forEach(itemData => {
           const newPosition = this.getItemPosition(itemData.progress);
   ...
   addItem(item: Product | Package): boolean{
       ...
       const pos = this.getItemPosition(0);
